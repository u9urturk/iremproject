...
import { deleteProductByProductId, getProducts } from '../../../firebase/productService';
    ...
    const productReaction = useCallback(
        async () => {
            try {
                ...
                const updatedProducts = await Promise.all(products.products.map(async (product) => {
                    const category = await getCategoryNameById(product.categoryId);
                    return {
            ...
            <table className="table table-md table-pin-rows table-pin-cols">
                ...
                <tbody>
                    {products.map((product, key) => {
                        ...
                        return <tr key={key} className={`hover:cursor-pointer hover:scale-x-95 border-none transition-transform ${(key % 2 === 0) ? "bg-base-200" : ""} `}>
